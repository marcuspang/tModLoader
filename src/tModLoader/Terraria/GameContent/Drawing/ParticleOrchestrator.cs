...
public class ParticleOrchestrator
{
	...
	private static FadingPlayerShaderParticle GetNewFadingPlayerShaderParticle() => new FadingPlayerShaderParticle();
	private static LittleFlyingCritterParticle GetNewPooFlyParticle() => new LittleFlyingCritterParticle();
	private static LittleFlyingCritterParticle GetNewNatureFlyParticle() => new LittleFlyingCritterParticle();
	private static ItemTransferParticle GetNewItemTransferParticle() => new ItemTransferParticle();
	private static FakeFishParticle GetNewFakeFishParticle() => new FakeFishParticle();
	...
	private static BloodyExplosionParticle GetNewBloodyExplosionParticle() => new BloodyExplosionParticle();
	private static ShockIconParticle GetNewShockIconParticle() => new ShockIconParticle();
	private static ItemTransferParticle GetNewItemTransferParticle_ScreenSpace() => new ItemTransferParticle();
	private static StormLightningParticle GetNewStormLightningParticle() => new StormLightningParticle();
	...
	private static void Spawn_StormLightning(ParticleOrchestraSettings settings)
	{
		...
		Main.NewLightning(instant: true, skipSound: true);
			SoundEngine.PlaySound(SoundID.InstantThunder with { Volume = MathHelper.Lerp(0.4f, 1f, Main.SceneState.outsideWeatherEffectIntensity) }, settings.PositionInWorld);
		...
		for (int i = point.X - 1; i <= point.X + 1; i++) {
			for (int j = point.Y - 1; j <= point.Y + 1; j++) {
				...
				if (!tileSafely.active())
					...
					int num3 = WorldGen.KillTile_GetTileDustAmount(fail: true, tileSafely, i, j);
	...
	private static void Spawn_BestReforge(ParticleOrchestraSettings settings)
	{
		...
		Asset<Texture2D> textureAsset = Main.Assets.Request<Texture2D>("Images/UI/Creative/Research_Spark");
		for (int i = 0; i < 8; i++) {
