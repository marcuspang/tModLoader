...
public partial class Main
{
	...
	/// </summary>
	internal bool hasFocus => instance.IsActive;

	/// <summary>The default damage variation percentage used when randomizing damage. Vanilla value is 15.</summary>
	public static float DefaultDamageVariationPercent => 15f;

	// Compatibility helper for 1.4.5 port patches that call Main.DamageVar directly.
	public static int DamageVar(float dmg, float luck = 0f)
	{
		if (DebugOptions.NoDamageVar)
			return (int)dmg;

		float varied = dmg * (1f + rand.Next(-15, 16) * 0.01f);
		if (luck > 0f) {
			if (rand.NextFloat() < luck) {
				float alt = dmg * (1f + rand.Next(-15, 16) * 0.01f);
				if (alt > varied)
					varied = alt;
			}
		}
		else if (luck < 0f && rand.NextFloat() < -luck) {
			float alt = dmg * (1f + rand.Next(-15, 16) * 0.01f);
			if (alt < varied)
				varied = alt;
		}
	...
	public static int DamageVar(float dmg, int percent, float luck)
	{
		if (DebugOptions.NoDamageVar)
			return (int)dmg;

		float scale = Math.Clamp(percent, 0, 100) / 100f;
		float varied = dmg * (1f + rand.Next(-100, 101) * 0.01f * scale);
		if (luck > 0f) {
			if (rand.NextFloat() < luck) {
				float alt = dmg * (1f + rand.Next(-100, 101) * 0.01f * scale);
				if (alt > varied)
					varied = alt;
			}
		}
	...
	/* Removed: RunOnMainThread now provided by vanilla 1.4.5 Main.cs
