...
public class WorldFile
{
	...
	public static bool IsValidWorld(string file, bool cloudSave) => GetFileMetadata(file, cloudSave) != null;
	...
	public static void LoadWorld()
	{
		...
		try {
			...
			try {
				...
				WorldIO.Load(Main.worldPathName, _isWorldOnCloud);
	...
	private static void ConvertOldTileEntities()
	{
		...
		foreach (Point item2 in list2) {
			...
			if (TileEntityType<TEWeaponsRack>.Place(item2.X, item2.Y) != -1 && flag2) {
				Item item = new Item();
				item.SetDefaults(num2);
				item.Prefix(prefix);
				TEWeaponsRack.TryPlacing(item2.X, item2.Y, item, 1);
			}
		}
	}
