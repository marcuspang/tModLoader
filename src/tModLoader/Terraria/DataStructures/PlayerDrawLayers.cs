...
public static partial class PlayerDrawLayers
{
	...
	{
		sbyte shoe = (sbyte)drawinfo.drawPlayer.shoe;
		if (shoe == 15)
			return true;
	...
	private static void DrawPlayer_21_Head_TheFace(ref PlayerDrawSet drawinfo)
	{
		...
		if (drawinfo.mountHandlesHeadDraw) {
			...
			if (drawinfo.drawPlayer.face == 5) {
				sbyte faceHead = (sbyte)drawinfo.drawPlayer.faceHead;
	...
	private static Vector2 GetCompositeOffset_BackArm(ref PlayerDrawSet drawinfo) => new Vector2(6 * (((drawinfo.playerEffect & SpriteEffects.FlipHorizontally) == 0) ? 1 : (-1)), 2 * (((drawinfo.playerEffect & SpriteEffects.FlipVertically) == 0) ? 1 : (-1)));
	private static Vector2 GetCompositeOffset_FrontArm(ref PlayerDrawSet drawinfo) => new Vector2(-5 * (((drawinfo.playerEffect & SpriteEffects.FlipHorizontally) == 0) ? 1 : (-1)), 0f);
