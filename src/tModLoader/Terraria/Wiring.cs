...
public static class Wiring
{
	...
	/// <summary>
	/// Helper method that calls <see cref="HitSwitch(int, int)"/> followed by <c>NetMessage.SendData(MessageID.HitSwitch...)</c>. This will cause the HitSwitch logic to run on all clients and the server, as intended.
	/// </summary>
	public static void HitSwitchAndSync(int i, int j)
	{
	...
	/// <summary>
	/// Contains shared code for tile interactions. Usually called on the server and all clients. Typically called in conjunction with <c>NetMessage.SendData(MessageID.HitSwitch...)</c> to have the method run everywhere else. (Or use the <see cref="HitSwitchAndSync(int, int)"/> helper method instead)
	/// </summary>
	public static void HitSwitch(int i, int j)
	{
		...
		if (!TileLoader.HitSwitch(i, j, Main.tile[i, j].TileType))
			return;
					...
					switch (num140) {
						...
						case 28:
							if (CheckMech(num139, num138, 30) && NPC.MechSpawn(num142, num143, 74)) {
								num144 = NPC.NewNPC(GetNPCSource(num139, num138), num142, num143 - 12, Utils.SelectRandom(Main.rand, new short[3] {
									74,
									297,
									298
								}));
						...
						case 40: {
							...
							*/
							var array2 = new List<int>(num150);
							...
							for (int num152 = 0; num152 < Main.maxNPCs; num152++) {
								...
								if (Main.npc[num152].active && (Main.npc[num152].type == 17 || Main.npc[num152].type == 19 || Main.npc[num152].type == 22 || Main.npc[num152].type == 38 || Main.npc[num152].type == 54 || Main.npc[num152].type == 107 || Main.npc[num152].type == 108 || Main.npc[num152].type == 142 || Main.npc[num152].type == 160 || Main.npc[num152].type == 207 || Main.npc[num152].type == 209 || Main.npc[num152].type == 227 || Main.npc[num152].type == 228 || Main.npc[num152].type == 229 || Main.npc[num152].type == 368 || Main.npc[num152].type == 369 || Main.npc[num152].type == 550 || Main.npc[num152].type == 441 || Main.npc[num152].type == 588)) {
									...
									if (Main.npc[num152].active && (NPCLoader.CanGoToStatue(Main.npc[num152], toKingStatue: true) ?? vanillaCanGo)) {
									/*
							...
							if (num151 > 0) {
								...
								NPCLoader.OnGoToStatue(Main.npc[num153], toKingStatue: true);
							}
							...
							*/
							...
							for (int num148 = 0; num148 < Main.maxNPCs; num148++) {
								...
								if (Main.npc[num148].active && (Main.npc[num148].type == 18 || Main.npc[num148].type == 20 || Main.npc[num148].type == 124 || Main.npc[num148].type == 178 || Main.npc[num148].type == 208 || Main.npc[num148].type == 353 || Main.npc[num148].type == 633 || Main.npc[num148].type == 663)) {
									...
									if (Main.npc[num148].active && (NPCLoader.CanGoToStatue(Main.npc[num148], toKingStatue: false) ?? vanillaCanGo)) {
									/*
									array[num147] = num148;
									*/
									array.Add(num148);
							...
							if (num147 > 0) {
								...
								NPCLoader.OnGoToStatue(Main.npc[num149], toKingStatue: false);
							}
