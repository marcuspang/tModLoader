...
public class WorldItem : Entity
{
	...
	public short makeNPC {
		get {
			return (short)inner.makeNPC;
		}
		set {
			inner.makeNPC = (short)value;
		}
	}
	...
	public override string ToString() => "[" + whoAmI + "]" + inner;
	...
	public bool OnlyNeedOneInInventory() => inner.OnlyNeedOneInInventory();
	public Color GetColor(Color newColor) => inner.GetColor(newColor);
	public Color GetAlpha(Color newColor) => inner.GetAlpha(newColor);
	public string AffixName() => inner.AffixName();
	...
	public void UpdateItem(int i)
	{
		...
		if (Main.netMode == 1) {
			...
			if (WorldGen.InWorld(p) && Main.tile[p.X, p.Y] == null) {
				...
				if (instanced && Main.GameUpdateCount % 10u == 0)
	...
	public IEntitySource GetItemSource_Misc(int itemSourceId) => new EntitySource_Misc(ItemSourceID.ToContextString(itemSourceId));
